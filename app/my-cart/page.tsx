...
export default function CartPage() {
  const { cart, removeFromCart, updateQuantity, cartTotal, placeOrder, cartCount, loading } = useOrder();
  ...
  const handlePayment = async (method: 'gpay' | 'phonepe') => {
    const orderToken = await placeOrder(customerName, customerPhone);
    if (!orderToken) {
        // Error is handled in the context, maybe just close the dialog
  ...
  const isPlaceOrderDisabled = !customerName.trim() || customerPhone.length !== 10 || loading;
